...
public abstract class ThriftOperator<E, T extends TBase> extends FunctionNode<E> {

  private final ThriftType thriftType;

  public ThriftOperator(
      Class<T> thriftClass,
      String exprText, ImmutableList<ASTNode> children)
      throws SemanticCheckFailure {
    super(exprText, children);
    this.thriftType = Type.thriftOf(thriftClass);
  ...
  private static boolean isDivergentTo(
    ...
    if (fieldType instanceof ThriftType && node instanceof MapOperator) {
      checkThriftFieldTypes((ThriftType) fieldType, node.getChildren());
      return false;
    }
