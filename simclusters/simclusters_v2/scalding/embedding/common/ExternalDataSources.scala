...
import graphstore.export.datasets.FlockBlocksEdgesScalaDataset
import graphstore.export.datasets.FlockFollowsEdgesScalaDataset
import graphstore.export.datasets.FlockReportAsAbuseEdgesScalaDataset
import graphstore.export.datasets.FlockReportAsSpamEdgesScalaDataset
  ...
  def getLocaleProducerSeedIdsFromUttEntityRecords(
    ...
    uttEntityRecordsSingleVersion.flatMap { uttEntityRecord: UttEntityRecord =>
      ...
      val localeProducerSeedIds: Seq[((TopicId, Language), Seq[UserId])] = validLocalizedUsers
        ...
        .mapValues(_.map(_._3).distinct)
        .toSeq

      localeProducerSeedIds.foreach {
    ...
    DAL
      ...
      .map { kv =>
        val consumed = kv.value.consumed
          .collect {
            case scoredString if scoredString.weight > 0.001 =>
    ...
    DAL
      ...
      .map { kv =>
        val produced = kv.value.produced
          .collect {
            case scoredString if scoredString.weight > 0.15 =>
              (getStandardLanguageCode(scoredString.item), scoredString.weight)
          }.collect {
            case (Some(language), score) => (language, score)
          }
