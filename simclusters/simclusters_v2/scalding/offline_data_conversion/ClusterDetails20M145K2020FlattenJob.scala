...
import com.twitter.simclusters_v2.hdfs_sources.SimclustersV2ClusterDetails20M145K2020ScalaDataset
import com.twitter.simclusters_v2.thriftscala.FlattenedClusterDetails

trait ClusterDetails20M145K2020FlattenBaseJob {
  def runJob(outputPath: String)(implicit dateRange: DateRange, tz: TimeZone): Execution[Unit] = {
    val clusterDetails: TypedPipe[FlattenedClusterDetails] = {
      DAL
        .readMostRecentSnapshot(
          SimclustersV2ClusterDetails20M145K2020ScalaDataset,
          dateRange.embiggen(Months(6))
        ).withRemoteReadPolicy(AllowCrossClusterSameDC)
        .toTypedPipe
        .collect {
          case KeyVal((ModelVersions.Model20M145K2020, clusterId), clusterDetails) =>
            FlattenedClusterDetails(
              clusterId = clusterId,
              clusterDetails = Some(clusterDetails)
            )
        }
    }

    clusterDetails
      .shard(5).writeDALSnapshotExecution(
        dataset = FlattenedSimclustersV2ClusterDetails20M145K2020ScalaDataset,
...
object ClusterDetails20M145K2020FlattenAdhocJob
    extends TwitterExecutionApp
    with ClusterDetails20M145K2020FlattenBaseJob {
  override def job: Execution[Unit] = Execution.withArgs { args =>
    ...
    val outputPath = args.getOrElse(
      "outputPath",
      "tmp/" + FlattenedSimclustersV2ClusterDetails20M145K2020ScalaDataset.logicalName)
    runJob(outputPath)
  }
}

object ClusterDetails20M145K2020FlattenProdJob
  ...
  override def scheduledJob: Execution[Unit] = {
    getArgs.flatMap { args =>
      val outputPath = args.getOrElse(
        "outputPath",
        FlattenedSimclustersV2ClusterDetails20M145K2020ScalaDataset.logicalName
      )
