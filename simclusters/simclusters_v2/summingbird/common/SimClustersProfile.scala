...
sealed trait SimClustersJobProfile extends SimClustersProfile {
  ...
  lazy val uuaSourceSubscribeId: String = {
    val jobTypeStr = jobType match {
      case JobType.PersistentTweet => "persistent_tweet_"
      case JobType.Tweet => ""
    }
    ...
    s"simclusters_v2_${jobTypeStr}uua_events_$prefix$env"
  }
}
...
object SimClustersProfile {
  ...
  final val VideoTweetJobProfile = SimClustersTweetProfile(
    env = Environment.Prod,
    alt = AltSetting.Alt,
    modelVersionStr = Model20M145K2020,
    entityClusterScorePath = simClustersCoreAltVideoCachePath,
    tweetTopKClustersPath = simClustersCoreAltVideoCachePath,
    clusterTopKTweetsPath = simClustersCoreAltVideoCachePath,
    coreEmbeddingType = EmbeddingType.LogFavBasedVideoTweet
  )
  ...
  final lazy val tweetJobProfileMap: Environment => Map[
    ...
    case Environment.Prod =>
      Map(
        ...
        (EmbeddingType.LogFavBasedVideoTweet, Model20M145K2020) -> VideoTweetJobProfile
      )
