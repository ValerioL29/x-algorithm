...
import com.twitter.simclusters_v2.thriftscala.NerKey
import com.twitter.simclusters_v2.thriftscala.PenguinKey
import com.twitter.simclusters_v2.thriftscala.SimClusterEntity
import com.twitter.simclusters_v2.thriftscala.TweetTextEntity
import com.twitter.taxi.util.text.TweetFeatureExtractor
import com.twitter.taxi.util.text.TweetTextFeatures
  ...
  def extractEntitiesFromText(
    ...
    val penguinEntities =
      extractTweetTextFeatures(
        ...
        .filter { s =>
          s.charAt(0) != '#' && !nerEntitySet.contains(s)
        }
