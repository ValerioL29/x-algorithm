...
object PersistentTweetStormJob {
  ...
  def apply(args: Args): StormConfig = {
    ...
    lazy val favoriteEventSource = TimelineEventSource(
      subscriberId = SubscriberId(profile.timelineEventSourceSubscriberId),
      parallelism = 120
    ).kafkaSource
    ...
    lazy val tweetStatusCountsService: Storm#Service[TweetId, StatusCounts] = {
      Storm.service(
        ObservedCachedReadableStore.from[TweetId, StatusCounts](
          TweetStatusCountsStore
            .tweetStatusCountsStore(stratoClient, "tweetypie/federated/favoritesCount.Tweet"),
          ttl = 1.minute,
          maxKeys = 10000,
    ...
    new StormConfig {
      ...
      override def transformConfig(config: Map[String, AnyRef]): Map[String, AnyRef] = {
        ...
        val TotalCPU = jLong(
          SourcePerWorker * 1 + FlatMapPerWorker * 6 + SummerPerWorker * 5 + 1
        )
