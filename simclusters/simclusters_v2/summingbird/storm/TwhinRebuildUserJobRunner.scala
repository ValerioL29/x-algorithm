...
import com.twitter.simclusters_v2.summingbird.stores.TwhinUserEmbeddingStore.prodRebuildUserRealTimePositiveStratoColumn
import com.twitter.simclusters_v2.summingbird.stores.TwhinUserEmbeddingStore.prodRebuildUserRealTimeNegativeStratoColumn
...
object TwhinRebuildUserJobRunner {
  def main(args: Array[String]): Unit = {
    GenericRunner(args, TwhinRebuildUserStormJob(_))
  }
}

object TwhinRebuildUserStormJob {
  ...
  def apply(args: Args): StormConfig = {
    ...
    lazy val serviceIdentifier =
      ServiceIdentifier(Configs.role, s"summingbird_twhin_rebuild_user_job", env, zone)

    lazy val stratoClient = ClientConfigs.stratoClient(serviceIdentifier)

    lazy val uuaEventSource =
      UnifiedUserActionsSourceScrooge(
        appId = AppId(s"twhin_rebuild_user_uua_events_prod"),
        parallelism = 150,
      ...
      Storm.service(
        TwhinTweetEmbeddingStore
          .cachedVersionedTweetEmbeddingStore(
            stratoClient,
            TwhinTweetEmbeddingStore.prodCachedRebuildTweetStratoColumn))
    }

    new StormConfig {

      val jobName: JobName = JobName(s"twhin_rebuild_user_job")
      ...
      override def graph: TailProducer[Storm, Any] = TwhinRebuildUserJob.generate[Storm](
        uuaEventSource,
