...
object TwhinTweetJobRunner {
  def main(args: Array[String]): Unit = {
    GenericRunner(args, TwhinTweetStormJob(_))
  }
}

object TwhinTweetStormJob {
  ...
  def apply(args: Args): StormConfig = {
    ...
    lazy val tweetEmbeddingStore: Storm#Store[TweetId, PersistentTwhinTweetEmbedding] = {
      ...
      lazy val mergeableStore = TwhinTweetEmbeddingStore
        .persistentTweetEmbeddingStore(
          stratoClient,
          TwhinTweetEmbeddingStore.prodPersistentTweetStratoColumn
        ).toMergeable(
    ...
    lazy val videoEmbeddingStore: Storm#Store[TweetId, PersistentTwhinTweetEmbedding] = {
      ...
      lazy val mergeableStore = TwhinTweetEmbeddingStore
        .persistentTweetEmbeddingStore(
          stratoClient,
          TwhinTweetEmbeddingStore.prodPersistentVideoStratoColumn
        ).toMergeable(
          mon = persistentTwhinTweetEmbeddingMonoid,
          fc = implicitly[FutureCollector]
        )
    ...
    lazy val userEmbeddingService: Storm#Service[UserId, TwhinTweetEmbedding] = {
      Storm.service(
        TwhinUserEmbeddingStore
          .userEmbeddingStore(stratoClient, TwhinUserEmbeddingStore.prodStratoColumn))
    }
    ...
    lazy val videoEmbeddingKafkaPipe: Storm#Sink[KeyedPersistentTwhinTweetEmbedding] = {
      Storm.sink(
        { keyedEmbedding =>
          kafkaEventPublisher
            .send(
              new ProducerRecord(
                "twhin_video_embedding_updates",
                keyedEmbedding.tweetId,
                keyedEmbedding.persistentEmbedding)).map(_ => ())
        }
      )
    ...
    new StormConfig {
      ...
      val jobName: JobName = JobName(s"twhin_tweet_job_$env")
      ...
      private val FlatMapPerWorker = 5
      private val SummerPerWorker = 5
      ...
      override def getNamedOptions: Map[String, Options] = Map(
        "DEFAULT" -> Options()
          ...
          .set(flatMapMetrics)
          .set(summerMetrics),
        TwhinTweetJob.NodeName.TweetUserEmbeddingFlatMapNodeName -> Options()
          .set(FlatMapParallelism(TotalWorker * FlatMapPerWorker)),
        TwhinTweetJob.NodeName.VideoUserEmbeddingFlatMapNodeName -> Options()
          .set(FlatMapParallelism(TotalWorker * FlatMapPerWorker)),
        TwhinTweetJob.NodeName.TweetEmbeddingSummerNodeName -> Options()
          .set(SummerParallelism(TotalWorker * SummerPerWorker))
          .set(FlushFrequency(30.seconds)),
        TwhinTweetJob.NodeName.VideoEmbeddingSummerNodeName -> Options()
          .set(SummerParallelism(TotalWorker * SummerPerWorker))
          .set(FlushFrequency(30.seconds)),
        TwhinTweetJob.NodeName.KeyedTweetEmbeddingMapNodeName -> Options()
          .set(FlatMapParallelism(TotalWorker * FlatMapPerWorker)),
        TwhinTweetJob.NodeName.KeyedVideoEmbeddingMapNodeName -> Options()
          .set(FlatMapParallelism(TotalWorker * FlatMapPerWorker)),
        TwhinTweetJob.NodeName.KeyedPersistentTwhinTweetEmbeddingWriteNodeName -> Options()
          .set(SummerParallelism(TotalWorker)),
        TwhinTweetJob.NodeName.KeyedPersistentTwhinVideoEmbeddingWriteNodeName -> Options()
          .set(SummerParallelism(TotalWorker)),
      )

      override def graph: TailProducer[Storm, Any] = TwhinTweetJob.generate[Storm](
        uuaEventSource,
        userEmbeddingService,
        tweetEmbeddingStore,
        videoEmbeddingStore,
        tweetEmbeddingKafkaPipe,
        videoEmbeddingKafkaPipe
      )
    }
  }
